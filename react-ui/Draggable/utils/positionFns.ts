...
export function getBoundPosition(draggable: Draggable, x: number, y: number): [number, number] {
  ...
  if (typeof bounds === 'string') {
    ...
    let boundNode: HTMLElement
    if (bounds === 'parent') {
      boundNode = node.parentNode as HTMLElement
    } else {
...
export function getControlPosition(e: MouseTouchEvent, touchIdentifier: (number | void), draggableCore: DraggableCore): ControlPosition | null {
...
function findDOMNode(draggable: Draggable | DraggableCore): HTMLElement {
  const node = ReactDOM.findDOMNode(draggable) as HTMLElement // React types it as Element | Text
  if (!node) {
